...
class StripeWH_Handler:
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        # Clean data in shipping dets
        for field, value in shipping_details.address.items():
            if value == "":
                shipping_details.address[field] = None

        # Update Profile information if save_info was checked in order form
        profile = None
        ...
        if username != 'AnonymousUser':
            ...
            if save_info:
                ...
                profile.default_county = shipping_details.address.staticfiles
                profile.save()
        ...
        if order_exists:
            ...
            return HttpResponse(
                content='Webhook recieved: {event["type"]} | SUCCESS: Verified order already in database',
                status=200
            )
        else:
            order = None
            try:
                ...
                if order:
                    order.delete()
                return HttpResponse(
                    content= 'Webhook recieved: {event["type"]} | ERROR {e}',
                    status=500)
        self._send_confirmation_email(order)
        return HttpResponse(
            content= 'Webhook recieved: {event["type"]}| SUCCESS: Created order in Webhook',
            status=200
        )
